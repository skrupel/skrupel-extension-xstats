...
function xstats_displayGraphsAsImage($gameId,$graphArray,$width,$height,$displayLegend,$statsPerRow,$isThumb) {
    ...
    foreach( $graphArray as $type) {
        ...
        echo( '<img class="bigStat" src="graph.php?gameid='.$gameId.'&type='.$type.'&width='.$width.'&height='.$height.'&outputtype=IMAGE&displayLegend='.$displayLegend.'" />');
        if ( $isThumb == "TRUE" ) {
            echo( '</a>' );
        }
                ...
                $displayStr=$displayStr."Gr&ouml;&szlig;te Flotte (Bewaffnet): <strong>".$maxValue."</strong> Schiffe ";
                    ...
                    $displayStr=$displayStr."Gr&ouml;&szlig;tes Imperium: <strong>".$maxValue."</strong> Planeten ";
                        ...
                        $displayStr=$displayStr."Gr&ouml;&szlig;te Bev&ouml;lkerung: <strong>".$maxValue."</strong> Kolonisten ";
                            ...
                            $displayStr=$displayStr."Gr&ouml;&szlig;ter Reichtum: <strong>".$maxValue."</strong> Cantox ";
                        }else if( $colName == 'lemincount' ) {
                                $displayStr=$displayStr."Gr&ouml;&szlig;ter Spritbesitz: <strong>".$maxValue."</strong> Lemin ";
                            }else if( $colName == 'factorycount' ) {
                                    $displayStr=$displayStr."Gr&ouml;&szlig;ter Fabrikbesitzer: <strong>".$maxValue."</strong> Fabriken ";
                                }else if( $colName == 'minescount' ) {
                                        $displayStr=$displayStr."Gr&ouml;&szlig;ter Minenbesitzer: <strong>".$maxValue."</strong> Minen ";
                                    }else if( $colName == 'sumcargohold' ) {
                                            $displayStr=$displayStr."Gr&ouml;&szlig;ter Frachtraum auf Frachtern: <strong>".$maxValue."</strong> KT ";
                                        }else if( $colName == 'lj' ) {
                                                $displayStr=$displayStr."Gr&ouml;&szlig;te Distanz geflogen: <strong>".$maxValue."</strong> Lj ";
                                            }
    //contains player colors
    include ('../../inc.conf.php');
    $displayStr = $displayStr.', erreicht durch <font color="'.$spielerfarbe[$playerIndex].'"><strong>'.xstats_getPlayerNick($playerId)."</strong></font> in Runde ".$turn."<br>";
    return($displayStr );
}
...
function xstats_displayFreigtherMostCargo( $gameId, $maxRows ) {
    ...
    $result = @mysql_query($query) or die(mysql_error());
    echo '<h4>Frachter mit dem gr&ouml;&szlig;ten Frachtvolumen:</h4>';
    echo '<table class="shiptable" border="0">';
    ...
    while ($counter++ < $maxRows && $row = mysql_fetch_array($result)) {
        ...
        echo '<td class="highlight"><img src="pie.php?perc='.$efficiency.'&size=30" /><span>'.$efficiency.'%</span></td>';
        //from-to
        echo '<td>Runde '.$row['sinceturn'].'-'.$untilTurn.'</td>';
        echo '</tr>';
    }
