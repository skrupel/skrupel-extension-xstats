...
abstract class AmChart {
	...
	public function getCode() {
		...
		$code .= ''
			. '<div class="amChart" id="chart_' . $this->id . '_div">' . "\n";

		if($this->title)
			$code .= '<div class="amChartTitle" id="chart_' . $this->id . '_div_title">' . $this->title . '</div>' . "\n";

		$code .= ''
			. '<div class="amChartInner" id="chart_' . $this->id . '_div_inner"><div id="chart_' . $this->id . '_flash">Chart loading ...</div></div>' . "\n"
			. '</div>' . "\n"
			...
			. 'swfobject.embedSWF("' . $this->getSwfPath() . '", "chart_' . $this->id . '_flash", "' . $width . '", "' . $height . '", "8", "", flashvars, params, {});' . "\n"
			. '// ]]>' . "\n"
			. '</script>' . "\n";
